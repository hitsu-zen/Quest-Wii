...
void PregameLytHandler::hijack_loadLevelNumber() {
	...
	OSReport("CP_ENTRANCE_PREG: %d\n", GameMgrP->eight.checkpointEntranceID);
	OSReport("PreviewID_PREG: %d\n", previewID);



	// this is not the greatest way to read a file but I suppose it works in a pinch
	char tplName[64];
	char chr = 'A' + previewID;
	sprintf(tplName, "/LevelSamples/%02d-%02d_%c.tpl", CurrentWorld+1, CurrentLevel+1, chr);
	OSReport("PreviewID_PREG_N: %s\n", tplName);
	static File tpl;
	if (tpl.open(tplName)) {
