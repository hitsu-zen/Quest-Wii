...
void dScKoopatlas_c::startLevel(dLevelInfo_c::entry_s *entry) {
	...
	OSReport("CP_ENTRANCE_CORE: %d\n", GameMgrP->eight.checkpointEntranceID);
	OSReport("PreviewID_CORE: %d\n", previewID);

	previewID = 0;
	if (GameMgrP->eight.checkpointWorldID == entry->worldSlot && GameMgrP->eight.checkpointLevelID == entry->levelSlot) {
...
static const wchar_t *completionMsgs[] = {
	...
	L"You've collected all of\nthe \x0B\x014F\xBEEF Rainbow Stars in\n",
	L"You have gotten every \x0B\x013B\xBEEF exit\nin",
	L"You have gotten everything\nin",
	L"You have collected all the\nnecessary \x0B\x014F\xBEEF stars to enter\nthe Special World!",
	L"You have collected all the \x0B\x014F\xBEEF Rainbow\nStars in the game!",
